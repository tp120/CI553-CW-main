                       ...
                       implements StockReadWriter
{
  ...
  public F_StockRW( String url )
  {
   ...
   /** public boolean reserveStock (String productNum, int quantity) throws StockException {
    try {
      ...
      int availableStock = theStock.getDetails(productNum).getQuantity();

      if (availableStock < quantity) {
        throw new StockException("Not enough stock for reservation");

        int newStockQuantity = availableStock - quantity;

        theStock.updateStock(productNum, newStockQuantity);

        return true;
      } catch(StockException e) {

        throw new StockException("Failed to reserve:" + e.getMessage());
      }
    }*/
  }
